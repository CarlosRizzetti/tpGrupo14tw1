...
public class ServicioDashboardTest {
  ...
  void deberiaRetornarListaDeDTOsConTimersActivos() {
    ...
    List<TimerDTO> resultado = servicioDashboard.obtenerTimersActivos(1L);
  ...
  void deberiaLanzarExcepcionCuandoIdEsNulo() {
    IdInvalido excepcion = assertThrows(
      IdInvalido.class,
      () -> servicioDashboard.obtenerTimersActivos(null)
    );
  ...
  void deberiaLanzarExcepcionCuandoIdEsCero() {
    IdInvalido excepcion = assertThrows(
      IdInvalido.class,
      () -> servicioDashboard.obtenerTimersActivos(0L)
    );
  ...
  void deberiaLanzarExcepcionCuandoIdEsNegativo() {
    IdInvalido excepcion = assertThrows(
      IdInvalido.class,
      () -> servicioDashboard.obtenerTimersActivos(-1L)
    );
    assertTrue(excepcion.getMessage().contains("positivo"));
    verifyNoInteractions(repositorioTimerMock);
  }
  ...
  @Test
  void deberiaLanzarExcepcionCuandoHayTimerNuloEnLaLista() {
    List<Timer> timersConNulo = new ArrayList<>();
    timersConNulo.add(null);

    when(repositorioTimerMock.obtenerTimersSegunEstado(1L, "activo")).thenReturn(timersConNulo);

    TimerMapeadoException excepcion = assertThrows(
      TimerMapeadoException.class,
      () -> servicioDashboard.obtenerTimersActivos(1L)
    );
    assertTrue(excepcion.getMessage().contains("Se encontró un timer nulo en la lista"));
  }
  ...
  void deberiaProcesarConIdMaximo() {
    ...
    assertDoesNotThrow(() -> servicioDashboard.obtenerTimersActivos(Long.MAX_VALUE));
  }
  ...
  void deberiaProcesarConIdMinimo() {
    ...
    assertDoesNotThrow(() -> servicioDashboard.obtenerTimersActivos(1L));
  }
  ...
  void deberiaProcesarListaConUnSoloTimer() {
    ...
    List<TimerDTO> resultado = servicioDashboard.obtenerTimersActivos(99L);
  ...
  void deberiaRechazarIdNegativoExtremo() {
    assertThrows(IdInvalido.class, () -> servicioDashboard.obtenerTimersActivos(Long.MIN_VALUE));
